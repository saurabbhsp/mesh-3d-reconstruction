...
from datasetReader.reader import get_raw_image_dataset
...
class ModelBuilder(object):
    ...
    def build_estimator(self, features, labels, mode, config=None):
        ...
        prediction_loss = self.get_loss(predictions, labels)
        loss = self.get_total_loss(prediction_loss)
    ...
    def visualize_raw_images(self, path_list):
        with tf.Graph().as_default():
            features = get_raw_image_dataset(path_list).make_one_shot_iterator().get_next()
            predictions = self.build_estimator(
                                    features, None,
                                    tf.estimator.ModeKeys.PREDICT).predictions
            targets = None
