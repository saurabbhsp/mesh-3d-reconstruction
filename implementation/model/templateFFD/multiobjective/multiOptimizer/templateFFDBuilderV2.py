...
from model.modelBuilder.mltbuilder import ModelBuilder
...
from model.shufflenet.shufflenet import shufflenet
...
class TemplateFFDBuilder(ModelBuilder):
    ...
    def base_model(self, features, mode):
        ...
        with tf.variable_scope('shufflenet'):
            features_template_selector = shufflenet(image, isTraining, 512)


        features_deformations = self.get_image_features(image, mode,
                                           **inference_params)

        """Generate the template deformations"""
        with tf.variable_scope('deformations'):
            features_deformations = tf.layers.flatten(features_deformations)
            dense_layers = inference_params.get("dense_layers", [512])
            for dense_layer in dense_layers:
                features_deformations = tf.layers.dense(features_deformations,
                                           dense_layer,
        ...
        """Generate the template selection vector"""
        with tf.variable_scope('template_selector'):
            features_template_selector = tf.layers.flatten(features_template_selector)
            dense_layers = inference_params.get("dense_layers", [512])
            for dense_layer in dense_layers:
                features_template_selector = tf.layers.dense(features_template_selector, dense_layer,
                                           activation=batch_normalized_activation(
                                            tf.nn.relu6, training=isTraining)
                                           )

            probs = tf.layers.dense(
                        features_template_selector, self.template_count, activation=tf.nn.softmax)
    ...
    def get_train_op(self, deformation_loss, selection_loss, step):

        deformation_optimizer = tf.train.AdamOptimizer(
            learning_rate=self.model_parameters.get('learning_rate', 1e-3))

        template_selector_optimizer = tf.train.AdamOptimizer(
            learning_rate=self.model_parameters.get('learning_rate', 1e-3))


        mobilenet_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                             scope='mobilenet')

        shufflenet_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                             scope='shufflenet')

        template_selector_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                                        scope='template_selector')


        deformation_variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES,
                                                        scope='deformations')

        template_selector_optimization = template_selector_optimizer.minimize(selection_loss,
                                                var_list = shufflenet_variables + template_selector_variables)

        deformation_optimization = deformation_optimizer.minimize(deformation_loss, step,
                                                var_list = mobilenet_variables + deformation_variables)

        return tf.group(template_selector_optimization, deformation_optimization)
    ...
    def get_selection_loss(self, predictions, ground_truth):
        ...
        template_loss = self.template_chamfer_loss(gamma, deformed_points, ground_truth)
        tf.summary.scalar('chamfer-template', template_loss, family='sublosses')
    ...
    def template_chamfer_loss(self, gamma, deformed_points, ground_truth):
        inferred_point_clouds = self.get_inferred_point_clound(tf.zeros_like(deformed_points))
        inferred_point_clouds = tf.unstack(inferred_point_clouds, axis=1)
        losses = [bidirectionalchamfer(inferred, ground_truth)
                  for inferred in inferred_point_clouds]
        losses = tf.stack(losses, axis=1)
    ...
    @property
    def template_loss_scaling_factor(self):
        return self.training_configuration.get('template_loss_scaling_factor', 0.2)
