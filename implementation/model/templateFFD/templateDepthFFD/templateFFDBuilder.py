...
class TemplateFFDBuilder(ModelBuilder):
        ...
        try:
            with tf.Graph().as_default():
                with tf.Session() as sess:
                    ...
                    transfer_saver = tf.train.Saver(coarse_params)
                    DepthEstimator.restore(transfer_saver, sess,
                                           self.depth_model_dir)
                    """End of loading pretrained model"""
    ...
    def base_model(self, features, mode):
        ...
        """Depth features"""
        depth_features = DepthEstimator.predict(features, 55, 74)
        depth_features = shufflenet(image, isTraining, 512)
        """Mobile net"""
        ...
        """Look for other best practices"""
        features = tf.concat([features, depth_features], axis = 1)
        """Template FC"""
    ...
    @property
    def category(self):
        return self.model_parameters["cat_desc"]
